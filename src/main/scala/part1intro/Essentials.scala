package part1intro

import java.util.concurrent.Executors
import scala.concurrent.{ExecutionContext, Future}
import scala.util.{Failure, Success, Try}

/**
 * User: patricio
 * Date: 25/6/21
 * Time: 13:00
 */
object Essentials {

  val aBoolean: Boolean = false

  // expressions Are evaluated to aa Value
  val anIfExpression = if (2 > 3) "bigger" else "smaller"

  // instructions va expression
  val theUnit = println("Something")

  // OOP
  class Animal

  class Cat extends Animal

  trait Carnivore {
    def eat(animal: Animal): Unit
  }

  // Inheritance model: extend <= 1 class, bit inherit from >= 0 traits
  class Crocodile extends Animal with Carnivore {
    override def eat(animal: Animal): Unit = println("Crunch")
  }

  // singleton
  object MySingleton

  // companion

  object Carnivore

  // generics
  class MuList[A]

  // method notation
  val three = 1 + 2
  val anotherThree = 1.+(2)

  // functional programming
  val incrementer: Int => Int = x => x + 1
  val incremented = incrementer(45)

  // map, flatMap, filter
  val processedList = List(1, 2, 3).map(incrementer)
  val aLongerList = List(1, 2, 3).flatMap(x => List(x, x + 1))

  // for-comprehensions
  // Cartesian product
  val checkerboard = List(1,2,3).flatMap(n => List("a", "b", "c").map(c => (n, c)))
  val anotherCheckerboard = for {
    n <- List(1,2,3)
    c <- List("a", "b", "c")
  } yield (n, c) // equivalent expression
  // aoptions and try
  val anOption: Option[Int] = Option(3) // Some(3)
  val doubledOption: Option[Int] = anOption.map(_ * 2)

  val anAttempr = Try(42) // Success(42) or Failure
  val aModifiedAttempt: Try[Int] = anAttempr.map(_ + 10);

  // pattern matching
  val anUnknown: Any = 45
  val ordinal = anUnknown match {
    case 1 => "First"
    case 2 => "Second"
    case _ => "unk"
  }

  val optiDescription: String = anOption match {
    case Some(value) => s"The option is not empty: $value"
    case None => "Th option is empty"
  }

  // Futures
  implicit val ex: ExecutionContext = ExecutionContext.fromExecutorService(Executors.newFixedThreadPool(8))
  val aFuture = Future {
    // a bit of code
    42
  }

  val anotherFuture = aFuture.map(_ + 1)


  // onComplete (async)
  aFuture.onComplete {
    case Success(value) => println(s"The meaning of life is: $value")
    case Failure(ex) => println(s"TMeaning value failed: $ex")
  }

  // partial function
  val aPartialFunction: PartialFunction[Int, Int] = {
    case 1 => 43
    case 8 => 56
    case 100 => 999
  }

  // some more advanced stuff
  trait HigherKindedType[F[_]]
  trait SequenceChecker[F[_]] {
    def isSequential: Boolean
  }

  val listChecker = new SequenceChecker[List] {
    override def isSequential: Boolean = true
  }


  def main(args: Array[String]): Unit = {
  }

}
